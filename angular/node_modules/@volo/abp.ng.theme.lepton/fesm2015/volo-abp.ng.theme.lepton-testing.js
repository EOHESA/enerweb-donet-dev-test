import { NgModule } from '@angular/core';
import { VALIDATION_ERROR_TEMPLATE, VALIDATION_INVALID_CLASSES, VALIDATION_TARGET_SELECTOR } from '@ngx-validate/core';
import { LEPTON_THEME_NAV_ITEM_PROVIDERS, LEPTON_THEME_STYLES_PROVIDERS, LEPTON_THEME_SETTING_TAB_PROVIDERS, ValidationErrorComponent, BaseThemeLeptonModule } from '@volo/abp.ng.theme.lepton';

class ThemeLeptonTestingModule {
    static withConfig() {
        return {
            ngModule: ThemeLeptonTestingModule,
            providers: [
                LEPTON_THEME_NAV_ITEM_PROVIDERS,
                LEPTON_THEME_STYLES_PROVIDERS,
                LEPTON_THEME_SETTING_TAB_PROVIDERS,
                {
                    provide: VALIDATION_ERROR_TEMPLATE,
                    useValue: ValidationErrorComponent,
                },
                {
                    provide: VALIDATION_INVALID_CLASSES,
                    useValue: 'input-validation-error',
                },
                {
                    provide: VALIDATION_TARGET_SELECTOR,
                    useValue: '.form-control',
                },
            ],
        };
    }
}
ThemeLeptonTestingModule.decorators = [
    { type: NgModule, args: [{
                exports: [BaseThemeLeptonModule],
                imports: [BaseThemeLeptonModule],
            },] }
];

/**
 * Generated bundle index. Do not edit.
 */

export { ThemeLeptonTestingModule };
//# sourceMappingURL=volo-abp.ng.theme.lepton-testing.js.map
